# Orquestador para FarmaciaKnopp Part1 y Part2 (paralelo total)
# Part1: Proteinas + Packs
# Part2: Vitaminas y Minerales (6 URLs)
...
PART1_SCRIPT = os.path.join(SCRAPERS_DIR, "FarmaciaKnopScraperPart1.py")
PART2_SCRIPT = os.path.join(SCRAPERS_DIR, "FarmaciaKnopScraperPart2.py")
...
def main():
    ...
    console.print(Panel(
        f"[bold green]FarmaciaKnopScraperRunner[/bold green]\n"
        f"Modo headless: {headless}\n"
        f"Inicio: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        f"Estrategia: Part1 (Proteinas+Packs) + Part2 (Vitaminas) en paralelo total",
