# Orquestador para Cruz Verde Part1 y Part2 (paralelo total)
# Part1: Proteinas + Snacks y Comida
# Part2: Energia + Bebidas Nutricionales
...
PART1_SCRIPT = os.path.join(SCRAPERS_DIR, "CruzVerdeScraperPart1.py")
PART2_SCRIPT = os.path.join(SCRAPERS_DIR, "CruzVerdeScraperPart2.py")
...
def main():
    ...
    console.print(Panel(
        f"[bold green]CruzVerdeScraperRunner[/bold green]\n"
        f"Modo headless: {headless}\n"
        f"Inicio: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        f"Estrategia: Part1 (Proteinas+Snacks) + Part2 (Energia+Bebidas) en paralelo total",
