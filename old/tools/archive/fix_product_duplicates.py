...
def fix_duplicates_db(engine, db_name):
    ...
    with engine.begin() as conn:
        ...
        print(f"[{db_name}] Encontrados {len(grupos)} grupos de duplicados.")
        ...
        try:
             ...
             # print(f"[{db_name}] Constraint 'uq_precio_fecha' eliminado (o no existía).")
        except Exception as e:
             print(f"[{db_name}] Advertencia al intentar borrar constraint: {e}")
        ...
        for row in grupos:
            ...
            # safe_print(f"[{db_name}] Procesando '{row.nombre_producto}' (Marca ID: {row.id_marca})")
        ...
        print(f"[{db_name}] Finalizado. Grupos procesados: {processed_count}. Productos eliminados: {deleted_count}.")
