# Deploy — ComparaFit Cloud Run Jobs (PowerShell)
...
> Abrir **PowerShell** (no Git Bash) para todos los comandos de `gcloud`.
> Los comandos de `docker` funcionan en cualquier terminal.
...
> `--platform linux/amd64` es obligatorio en Windows.
...
```powershell
"nuevo_valor" | gcloud secrets versions add NOMBRE_SECRETO --data-file=-
```
